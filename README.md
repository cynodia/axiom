...
[`docs/COMPATIBILITY.md`](docs/COMPATIBILITY.md). This documentation describes 1.0.0.
...
42. **Long-running orchestration is a `WorkflowDef`, not application state-machine code.** Six portable steps (`action`, `wait-event`, `timer`, `branch`, `complete`, `fail`), no script body, no mutable blob. A workflow instance has a durable control position that survives process death, authority failover, retries, timer and event delivery: exactly-once logical transition per activation, no event-wait registration gap, fenced stale owners, idempotent start, cancellation that is durable but not rollback. The same logical result at 1 or N authorities. Full model: [`docs/WORKFLOWS.md`](docs/WORKFLOWS.md).
...
npm's `latest` tag points at the current release, so the plain commands above install it.
**There is no `alpha` dist-tag** — it was removed once it stopped tracking releases, and
`npm install @cynodia/axiom@alpha` fails with a 404. Pin the exact version instead when one
is needed: `npm install @cynodia/axiom@1.0.0`.
