...
- Executable Server IR is **normalized** semantic input. Compilation lowers authoring conveniences into the canonical executable form; an authority evaluates that form and MUST NOT depend on an authoring-only representation the contract defines a normalized form for. `ActionDef.guards` is lowered into aligned `preconditions` / `failureModes` preserving meaning and order, and the authority evaluates the lowered representation only — it does not also execute `guards[]`.
- A surface that accepts serialized semantic input — deserialized Server IR, a conformance fixture, a persisted or network-provided artifact — MUST perform structural admission before semantic execution, and MUST be **total** over that input: a malformed value inside a semantic-node collection (`null`, a primitive, a non-object), a non-array required collection, a non-object document, or a non-normalized guard representation produces a structured `SERVER_IR_*` diagnostic with zero mutation and zero external effect — never an uncaught host-language exception. A language-level type declaration is not runtime validation. Full model: [`AUTHORITY.md`](AUTHORITY.md#server-ir-admission).
