...
Phase 22 against published `0.14.0-alpha.3` produced:
...
Phase 22 directly demonstrated this failure mode in `0.14.0-alpha.3`.
...
must remain unchanged from `0.14.0-alpha.3` unless the previous value was itself demonstrably incorrect.
...
Baseline proof should demonstrate old behaviour if harness supports running published `0.14.0-alpha.3`:
...
Compare compatibility behaviour with `0.14.0-alpha.3`.
...
Because `0.14.0-alpha.3` has already been externally tested and contains a release-blocking semantic compatibility defect, do not overwrite/reuse it.
...
Decide explicitly whether workflow instances created by `0.14.0-alpha.3` contain sufficient durable identity for `alpha.2` to determine compatibility.
