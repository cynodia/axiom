...
| 14 | `packages/demo/src/order-management.ts` — Customer/Product/Order/OrderLine + Principal, 4 relationships, 1 read policy, 8 queries (Orders with status+search+date-range filter, cursor, Customer join, OrderSummary projection; OrderDetail; OrderLines with Product join + computed lineTotal; 4 dashboard aggregates incl. grouped revenue-by-status; CustomerHistory), 2 provider-record mutations (confirm/cancel). `generateOrderManagementDataset({orders,...})` deterministic generator. `order-management.test.ts` (9): Orders screen end to end, case-insensitive search, dashboard aggregates all provider-side (0 row calls), customer A never sees B (incl. hostile customerId), mutation-from-query, computed lineTotal, §106 no-escape-hatch source scan, client store pagination without touching a cursor string. Compiles to `axiom.server.v6`. No UI views — visual screens noted as a limit. | ✅ committed |
