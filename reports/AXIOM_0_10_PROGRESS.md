...
| 6c | server execution: load `provider-record` rows via `loadByIdentity` into an in-transaction staging state, rewrite the operation targets to `collection-item`, run through the unchanged engine, then `provider.applyMutations` the diff atomically post-commit; cross-record constraint test; `confirmOrder` demo action | ⬜ |
