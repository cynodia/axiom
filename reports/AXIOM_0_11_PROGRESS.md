...
| 1 | `core` semantic schema identity + deterministic fingerprint: `graph.schemaVersion`, `schemaFingerprint(graph)` over persistence-relevant structure only, `SCHEMA_FINGERPRINT_VERSION`, stable-id canonicalization; `schema-identity.ts` + tests | ⬜ |
